...
def calculate_technical_indicators(df):
    ...
    df['%K'] = 100 * (df['Close'] - df['Low'].rolling(14).min()) / (df['High'].rolling(14).max() - df['Low'].rolling(14).min())  # Stochastic Oscillator
...
btc_data = yf.download('NEAR-USD', start='2021-01-01', end=end_date)
...
plt.xlabel('Date')
plt.ylabel('Price')
plt.title('NEAR Protocol Price Prediction with Enhanced LSTM')
plt.legend()
plt.grid()
plt.show()